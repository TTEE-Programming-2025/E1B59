...
int FunctionE(){
 printf("Continue?(Y/N) : ");
  ...
  if(Key != 78 && Key != 89 && Key != 110 && Key != 121){ 
   system("cls");
   printf("Y or N: ");
   Key=getch();
  }
...
//當我完成這份學生成績管理系統的程式後，我深刻體會到資料結構與邏輯思考在實作中的重要性。
//這份程式雖然功能簡單，卻涵蓋了多項實用概念，例如陣列的應用、字串比對、成績排序及選單操作等，讓我對C語言的操作有更深的了解。
//過程中也遇到不少挑戰，例如輸入錯誤的處理、平均成績的計算與排序邏輯的設計，這些都考驗我對程式流程的掌握。
//最終能夠完成一個可執行的系統，讓我獲得滿滿的成就感，也更加確定繼續學習程式設計的方向與動力。
